...
- 全称是 `Global Interpreter Lock`(全局解释器锁)，是`CPython`的产物
    - 一个进程中只有一个GIL，拿到GIL的线程才能执行程序
    - 在多线程环境下，每个线程的执行方式：获取GIL、执行代码、释放GIL
    - 因此一个进程中永远只能同时执行一个线程，保证数据安全

- 释放GIL锁的时机
    - 基于opcode计数，达到一定计数后自动释放GIL
    - 基于时间片计数，达到一定计数后自动释放GIL

- GIL锁的缺点
    - 对多线程效率有较大的影响
    - 每次释放GIL，线程进行锁竞争、切换线程，会消耗资源

- GIL锁存在竞争、切换线程的情况，会消耗资源，为什么还需要多线程
    - 单线程下有 IO 操作会进行 IO 等待，造成不必要的时间浪费
    - 而开启多线程能在线程 A 等待时，自动切换到线程 B，可以不浪费 CPU的资源，从而能提升程序执行效率
    - 所以对于IO 密集型程序(文件处理、网络爬虫等)，多线程能够有效提升效率

- 什么时候用GIL
    - 因为GIL的存在，只有IO Bound场景下得多线程会得到较好的性能
    - 对并行计算性能较高的程序可以考虑把核心部分也成C模块，或者用其它语言实现
...
- 参一函数的参数：
    - `map`/`filter` 参一位置的函数，只能接受一个参数
    - `reduce`       参一位置的函数，只能接受两个参数

- 对可迭代对象的作用方式：
    - `map` 参一函数依次作用于序列的每个元素，参一函数作用的结果依次添加到新序列，最后返回新序列
    - `reduce` 参一函数作用于序列的第一个和第二个元素后得到返回值，再将这个返回值与下一个元素计算，直到最后一个元素（累积计算）
    - `filter` 参一函数依次作用于序列的每个元素并判断返回值是否为True，如果True，将序列中的元素添加到新序列，最后返回新序列

- 返回值的类型
    - `map`、`filter` 返回值是一个迭代器(py3)/列表(py2)
    - `reduce` 返回值和参一函数的返回值类型相同

- 其它
    - 在python3中 `reduce` 已经从BIF中删除了，需要从`functools`中导入
    - `filter` 参一函数如果是`None`，那么直接返回序列中非空元素（0,'',[],(),{},None,False）组成的序列
    - `map` 返回新序列的长度与原序列长度相等，新序列中的每一个值是原序列中每个值计算得到的
    - `filter` 返回新序列的长度小于等于原序列长度，新序列中的每一个值都`in`（包含于）原序列
...
### 并发和并行
- 并行和并发从宏观上来讲都是同时处理多路请求的概念
- **并行**是指两个或者多个事件在**同一时刻**发生
- **并发**是指两个或多个事件在**同一时间间隔**内发生
- 在单核 CPU 下的多线程其实都只是并发，不是并行
